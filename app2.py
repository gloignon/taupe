...
def process_zip_initial(zip_file):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        df = pd.DataFrame({"Document Name": txt_files, "Word Count": word_counts})
...
def process_zip_and_search(zip_file, keywords_text):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        df_keywords.rename(columns={"index": "Document Name"}, inplace=True)
        
        # Replace 0 frequencies with empty strings
        df_keywords.replace(0, "", inplace=True)
        
        # Create a DataFrame with document names and word counts
        df_word_counts = pd.DataFrame({"Document Name": base_names, "Word Count": word_counts})
        
        # Merge the Word Count DataFrame with the Keywords DataFrame on "Document Name"
        final_df = pd.merge(df_word_counts, df_keywords, on="Document Name")
...
with gr.Blocks() as demo:
    with gr.Row():
        ...
        zip_file_input = gr.File(label="Upload ZIP file containing .txt files (supports subfolders)")
    
    with gr.Row():
        # Textbox for entering keywords
        keywords_input = gr.Textbox(label="Enter keywords (one per line)", placeholder="keyword1\nmultiword expression\n...", lines=10)
    
    with gr.Row():
        # Button to trigger keyword search
        search_button = gr.Button("Search for Keywords")
    
    # Output the final results table after the search button
    with gr.Row():
        result_table = gr.DataFrame(
            label="Document Names and Keyword Frequencies", 
            col_count=(1, "dynamic"), 
            interactive=False, 
            max_height=300  # Set a fixed height
        )
    
    # Button to trigger the Excel export (disabled by default)
    with gr.Row():
        export_button = gr.Button("Export to Excel", interactive=False)  # Initially disabled
        download_link = gr.File(label="Download Excel File")
    ...
    # Action to update the table with keywords and results and enable the export button
    def enable_export(df):
        return df, gr.update(interactive=True)  # Enable the export button when results are ready
    
    search_button.click(
        fn=process_zip_and_search, 
        inputs=[zip_file_input, keywords_input], 
        outputs=[result_table, export_button], 
        _js=enable_export  # Enable the export button
    )
